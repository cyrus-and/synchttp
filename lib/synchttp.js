...
function request(method, options, hasBody, message, contentType) {
...
// synchronous send through a WebSocket
function wsSend(ws, message, contentType) {
    var future = new Future();
    if (typeof ws !== 'undefined') {
        var stringMessage = bodyUtils.stringify(contentType, message);
        ws.send(stringMessage, function (err) {
            if (err) {
                future.return(err);
            } else {
                future.return();
...
function Synchttp() {
    ...
    self.post = wrap(function (message) {
        ...
        return request('POST', options, true, message, contentType);
    });

    self.put = wrap(function (message) {
        var options = generateHttpOptions.call(self);
        var contentType = self.parameters.httpContentType;
        return request('PUT', options, true, message, contentType);
    });
    ...
    self.wsSend = wrap(function (message) {
        return wsSend(self.ws, message, self.parameters.wsContentType);
    });

    self.wsReceive = wrap(function () {
        return wsReceive(self.ws, self.parameters.wsContentType);
    });
}
